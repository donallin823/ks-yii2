...
class KsUtils
{
    ...
    // 生成token
    public static function newToken($userId, $secretKey)
    {
        return md5(md5($userId . $secretKey) . time());
    }

    // 生成唯一邀请码
    public static function genInviteCode($user_id)
    {
        ...
        if (strlen($code) < 5) {
            $code .= self::_mtRand(5 - strlen($code));
        }
    ...
    public static function getUniqueId($salt, $size = 'big')
    {
        ...
        $unique_id = '';
        if ($size == 'mini') {
            $unique_id = $increase_id;
        } elseif ($size == 'small') {
            $unique_id = "{$version}{$increase_id}{$sys_id}";
        } else {
            $unique_id = "{$version}{$date_time}{$increase_id}{$sys_id}";
        }
        return $unique_id;
    }
    ...
    {
        $data = !is_null($arr_key) ? (empty($array[$arr_key][$param]) ? null : $array[$arr_key][$param]) : (is_null($array) ? $param : (empty($array[$param]) ? null : $array[$param]));
//        $data = is_null($array) ? $param : (empty($array[$param]) ? null : $array[$param]);
     ...
     *              'rows' => $rows, // 需要加载的数据集合
     *              'file_dir' => '/data/dir' // 需要下载的地址，不传时实时下载
     *              'file_encode' => 'UTF-8'
     *          ]
     * @return bool
     */
    public static function exportFile($options = [])
    {
        ...
        $file_dir = !empty($options['file_dir']) ? strval($options['file_dir']) : '';
        $file_head = !empty($options['fields']) ? $options['fields'] : [];
        $file_encode = !empty($options['file_encode']) ? $options['file_encode'] : 'UTF-8';
        ...
        $file_head = array_map(function ($v) use ($file_encode) {
            return iconv('utf-8', $file_encode, $v);
        }, $file_head);
        $limit = 100000; // 文件行数限制
        ...
        for ($i = 0; $i < ceil($data_count / $limit); $i++) {
            ...
            for ($j = 0; $j < $limit; $j++) {
                ...
                $arr_row = array_map(function ($v) use ($file_encode) {
                    return iconv('utf-8', $file_encode, $v);
                }, $arr_row);
        ...
        if ($data_count <= $limit) { // 生成csv文件
            ...
            if (!empty($file_dir)) {
                ...
                $new_file = rtrim($file_dir, '/') . '/' . $csv_file;
                copy($csv_file, $new_file);
                unlink($csv_file);
                return $new_file;
                exit;
            }
            ...
            header('Content-Type:vnd.ms-excel;charset=' . $file_encode);
            ...
            if (!empty($file_dir)) {
                ...
                $new_file = rtrim($file_dir, '/') . '/' . $zip_file;
                copy($zip_file, $new_file);
                unlink($zip_file);
                return $new_file;
                exit;
            }
            ...
            header("Content-Type: application/zip;charset=' . $file_encode");
