...
class SqlBuilder
{
    ...
    public function update($table, $columns, $condition)
    {
        $params = [];
        foreach ($columns as $key => $value) {
            if (preg_match("/(?P<param>\w+)(?P<symbol>[\+\-\/\%])(?P<figure>\d+)/", $value, $match)) { // 支持自增语句
                $params[] = intval($match['figure']);
                $columns[$match['param']] = $this->quoteColumn($match['param']) . "=`{$match['param']}`{$match['symbol']}?";
            } else {
                $params[] = $value;
